...
async function get_channel(slug, opts) {
  return await GET(opts.host + "channels/" + slug + "?per=100", opts.auth);
}

async function hack_refresh(slug, opts) {
  return create_block(slug, { content: "TBR" }, opts).then((res) =>
    disconnect_block(slug, res.id, opts)
  );
}
...
async function get_block(slug, opts) {
  return await GET(opts.host + "blocks/" + slug, opts.auth);
}
...
async function create_block(channel_slug, request_data, opts) {
  return await POST(
    opts.host + "channels/" + channel_slug + "/blocks",
    request_data,
    opts.auth
  );
}
...
async function update_block(id, request_data, opts) {
  return await PUT(opts.host + "blocks/" + id, request_data, opts.auth);
}
...
async function disconnect_block(channel_slug, id, opts) {
  ...
  return DELETE(
    opts.host + "channels/" + channel_slug + "/blocks/" + id,
    opts.auth
  );
}
